...
public class TC07_AdminPortal_LoginTest extends AdminPortalLoginLogoutPage {
...

	{
		...
		//Then do actions on UI
		 Login(testData);
		 
		//set test data back to excel
	...
	public void Login(List<String> testData) throws IOException, AWTException
	{
		...
		try {
			...
			//reporting
			if(title.equalsIgnoreCase(testData.get(3)))
	        {
	        	TestBase.classAInstance.logReport("Pass","Sucessful Login Drug Reimbursement","Succesfully able to Verify Sucessful Login of Drug Reimbursement Admin Portal");
			}
			else
			{
				TestBase.classAInstance.logReport("Fail","Sucessful Login Drug Reimbursement","Failed to Verify Sucessful Login of Drug Reimbursement Admin Portal");
			}	
		} catch (InterruptedException e) {
