...
public class TC03_DeactivationLetterScenario_VerifyNoLongerEligibleLetter extends AdminPortalLoginLogoutPage {
	...
	public void CloseBrowser(ITestResult result) throws IOException, AWTException {
		if(result.getStatus()== ITestResult.SUCCESS){
			TestBase.classAInstance.logReport("Pass", "Deactivation letter scenario", "Succesfully verify Deactivation letter ");
			} else {
			TestBase.classAInstance.logReport("Fail", "Deactivation letter scenario", "Failed to verify Deactivation letter");
			}
		closeBrowser();
	...
	public void SearchPatient() throws IOException, AWTException, InterruptedException
	{
		...
		if (afm.EnrollmentDeactivation().getText().equalsIgnoreCase("Enrollment Deactivation") ) {
			TestBase.classAInstance.logReport("Pass", "Enrollment Deactivation letter", "Succesfully verified Enrollment Deactivation letter in admin");
		} else {
			TestBase.classAInstance.logReport("Fail", "Enrollment Deactivation letter", "Failed to verified Enrollment Deactivation letter in admin");
		}  
