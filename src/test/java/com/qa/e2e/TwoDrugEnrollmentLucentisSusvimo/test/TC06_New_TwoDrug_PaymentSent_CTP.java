...
public class TC06_New_TwoDrug_PaymentSent_CTP extends TestBase {
...
/*	for (int loop2 = 1; loop2 <= rwCount; loop2++) {
	...
	System.out.println("TotalRowsPayment is:" +TotalRowsPayment);
//	List<WebElement> columnspayment;
    ...
    for (int loop1 = 1; loop1 <= TotalRowsPayment; loop1++) {	
	  ...
	  if(MemberIDScreen.equalsIgnoreCase(testData.get(0)) && PaymentAmount.contains("11,995.00"))
	  {
    ...
    if( driver.findElement(By.xpath("//*[contains(text(),'Approve Payments')]")).isDisplayed())
    {
    	TestBase.classAInstance.logReport("Pass", "Max Accum payment", "Succesfully sent for payment");
	} else {
	TestBase.classAInstance.logReport("Fail", "Max Accum payment", "Failed to sent for payment");
	 ...
	 Thread.sleep(4000);
		...
		if( driver.findElement(By.xpath("//*[contains(text(),'Confirm')]")).isDisplayed())
		    {
		    	TestBase.classAInstance.logReport("Pass", "Payment", "Succesfully Clicked on Confirm");
			} else {
			TestBase.classAInstance.logReport("Fail", "Payment", "Failed to Click on Confirm");
