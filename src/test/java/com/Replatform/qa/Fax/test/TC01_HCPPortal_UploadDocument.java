...
public class TC01_HCPPortal_UploadDocument extends HubHomeLoginLogoutPage {
	...
	{		String title = driver.getTitle();
		...
		try {
			...
			List<String> testData = gst.FaxGetPhysicianNewPageRequiredFields(Key, rowNum); //Same method for all Provider
			...
			eft.SearchByMemberID().sendKeys(testData.get(1).trim());
			Thread.sleep(2000);
	...
	public void CloseBrowser(ITestResult result) throws IOException, AWTException {
		if(result.getStatus()== ITestResult.SUCCESS){
			TestBase.classAInstance.logReport("Pass", "Fax", "Succesfully uploaded a fax");
			} else {
			TestBase.classAInstance.logReport("Fail", "Fax", "Failed to upload a fax");
			}
		closeBrowser();
