...
public class ProviderGroups_BillingOffice_PatientsTab extends TestBase {
	
	TestUtil testUtil;
	Xls_Reader reader = new Xls_Reader(testUtil.TESTDATA_SHEET_PATH);
	
	public List<String> GetPGBOAndPatientMappingTestData(String Key, int rowNum)
	{
		...
		{	
			...
			testData.add(11, row.getCell(12).toString());//CardID
			testData.add(12, row.getCell(21).toString());//CardID
	...
	public List<String> GetPGBOPatientDataForPatientAndCardInfo(String Key, int rowNum)
	{
		List<String> testData = new ArrayList<String>();
		String sheetname = "Regression";
		reader.getSheet(sheetname);
		...
		if(Key.equalsIgnoreCase(row.getCell(0).toString()))
		{
			...
			testData.add(0, row.getCell(12).toString());//PatientNameValue						
			testData.add(1, row.getCell(13).toString());//CityValue
			testData.add(2, row.getCell(14).toString());//stateValue
			testData.add(3, row.getCell(15).toString());//ZipValue
			testData.add(4, row.getCell(16).toString());//MemberIDValue
			testData.add(5, row.getCell(17).toString());//PartnerPatientIDValue
			testData.add(6, row.getCell(18).toString());//CardIDValue
			testData.add(7, row.getCell(19).toString());//providernametypevalue
			testData.add(8, row.getCell(20).toString());//activation date
